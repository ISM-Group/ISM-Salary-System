...
exports.getMyPayslip = exports.getMyLoans = exports.getMyAttendance = exports.getMySalaryHistory = exports.getMyProfile = void 0;
 ...
 */
// PUBLIC_INTERFACE
async function resolveEmployeeForUser(req) {
    if (!req.user)
        return null;
    // Primary: direct user_id link
    let employee = await (0, db_1.queryOne)(`SELECT e.id, e.employee_id AS employeeCode, e.full_name AS fullName, e.email,
    ...
    employee = await (0, db_1.queryOne)(`SELECT e.id, e.employee_id AS employeeCode, e.full_name AS fullName, e.email,
            e.salary_type AS salaryType, e.base_salary AS baseSalary,
            d.name AS departmentName, r.name AS roleName
     FROM employees e
     LEFT JOIN departments d ON d.id = e.department_id
     ...
     LIMIT 1`, [req.user.username, req.user.full_name]);
...
const getMySalaryHistory = async (req, res) => {
    ...
    const rows = await (0, db_1.query)(`SELECT id, month, daily_wage_total AS gross, bonus, advance_deductions AS advanceDeductions,
            loan_deductions AS loanDeductions, total_salary AS totalSalary, status
     FROM salary_calculations
     WHERE employee_id = ?
     ORDER BY month DESC
     LIMIT 24`, [employee.id]);
    res.json({
        data: rows.map((r) => ({
            ...r,
...
exports.getMySalaryHistory = getMySalaryHistory;
// PUBLIC_INTERFACE
/**
...
const getMyLoans = async (req, res) => {
    ...
    const rows = await (0, db_1.query)(`SELECT id, loan_amount AS loanAmount, remaining_balance AS remainingBalance,
            status, repayment_mode AS repaymentMode, daily_repayment_amount AS dailyRepaymentAmount,
            created_at AS createdAt
     FROM loans WHERE employee_id = ? ORDER BY created_at DESC`, [employee.id]);
    res.json({
        data: rows.map((r) => ({
            ...r,
...
const getMyPayslip = async (req, res) => {
    ...
    const { generatePayslip } = await Promise.resolve().then(() => __importStar(require('./exports.controller')));
    await generatePayslip(req, res);
};
exports.getMyPayslip = getMyPayslip;
