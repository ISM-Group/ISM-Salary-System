...
import { isHolidayForEmployee } from './holidays.controller';
 ...
 */
// PUBLIC_INTERFACE
export const generateDailyReleases = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const holidayEligibleEmployees = await query<any>(
      `SELECT
         e.id AS employeeId,
    ...
    for (const emp of holidayEligibleEmployees) {
      const holiday = await isHolidayForEmployee(emp.employeeId, date);
      if (holiday && holiday.type === 'PAID') {
        ...
        const id = generateId();
        await execute(
          `INSERT INTO daily_salary_releases
           ...
           VALUES (?, ?, ?, ?, 0, 0, ?, 'PENDING', 'PRESENT', NOW(), NOW())`,
          [id, emp.employeeId, date, fullDailyWage, fullDailyWage]
        );
        holidayGenerated++;
        generated++;
      }
    ...
    const presentEmployees = await query<any>(
      `SELECT 
         e.id AS employeeId,
         e.full_name AS employeeName,
         e.role_id AS roleId,
    ...
    for (const emp of presentEmployees) {
      ...
      try {
        const loanRows = await query<any>(
          `SELECT COALESCE(SUM(l.daily_repayment_amount), 0) AS totalDailyLoan
           FROM loans l
           WHERE l.employee_id = ?
             AND l.status = 'ACTIVE'
             AND l.repayment_mode = 'DAILY'
             AND l.daily_repayment_amount > 0`,
          [emp.employeeId]
        );
      ...
      const advRows = await query<any>(
        `SELECT COALESCE(SUM(a.amount), 0) AS totalAdvance
      ...
      await execute(
        `INSERT INTO daily_salary_releases 
         (id, employee_id, release_date, daily_wage, loan_deduction, advance_deduction, net_amount, status, attendance_status, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'PENDING', 'PRESENT', NOW(), NOW())`,
        [id, emp.employeeId, date, dailyWage, dailyLoanDeduction, advanceDeduction, netAmount]
      );
      generated++;
    }
    ...
    const parts: string[] = [];
    if (generated > 0) parts.push(`Generated ${generated} daily release record(s)`);
    if (holidayGenerated > 0) parts.push(`including ${holidayGenerated} holiday pay record(s)`);
 ...
 */
// PUBLIC_INTERFACE
export const getDailyReleases = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
 ...
 */
// PUBLIC_INTERFACE
export const releaseDailySalary = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;

    const existing = await queryOne<any>(
      `SELECT id, status, employee_id, release_date, loan_deduction FROM daily_salary_releases WHERE id = ?`,
      [id]
    );
 ...
 */
// PUBLIC_INTERFACE
export const releaseAllDailySalaries = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { date } = req.body as { date?: string };
    ...
    const pendingReleases = await query<any>(
      `SELECT id, employee_id, loan_deduction FROM daily_salary_releases
       WHERE release_date = ? AND status = 'PENDING'`,
      [date]
    );
 ...
 */
// PUBLIC_INTERFACE
export const deleteDailyRelease = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;

    const existing = await queryOne<any>(
      `SELECT id, status, employee_id, release_date FROM daily_salary_releases WHERE id = ?`,
      [id]
    );
    ...
    await execute(`DELETE FROM daily_salary_releases WHERE id = ?`, [id]);
 ...
 */
// PUBLIC_INTERFACE
export const getEmployeeDailyReleases = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
 ...
 */
async function deductLoanBalances(employeeId: string, totalDeduction: number): Promise<void> {
  try {
    // Find all active daily-repayment loans for the employee
    const activeLoans = await query<any>(
      `SELECT id, daily_repayment_amount, remaining_balance
       FROM loans
       WHERE employee_id = ?
         AND status = 'ACTIVE'
