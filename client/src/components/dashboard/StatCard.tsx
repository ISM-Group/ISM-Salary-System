    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <p className="break-words text-2xl font-semibold tracking-tight text-foreground sm:text-3xl">{value}</p>
          {description && (
            <p className="line-clamp-2 text-[10px] text-muted-foreground sm:text-xs">{description}</p>
          )}
          {trend && (
        ...
        <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-md bg-accent/20 sm:h-12 sm:w-12">
          <Icon className="h-5 w-5 text-accent dark:text-accent sm:h-6 sm:w-6" />
        </div>
      </div>
    </motion.div>
  );
